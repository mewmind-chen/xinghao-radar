...
function recordRun(
  ...
  result.runs.push(
    response
      ? {
          provider: response.channel ?? provider.name,
...
function inquiryCustomer(line: string, mpnIndex: number): string | null {
  const token = "[\\p{L}\\p{N}._&+()（）-]{1,60}";
  const labelled = line.match(
    new RegExp(`(?:^|[\\s|,，;；])客户\\s*(?:[:：]\\s*|\\s+)(${token})(?=$|[\\s|,，;；])`, "u"),
  );
  if (labelled?.[1]) return labelled[1].trim();

  // A compact leading token such as “客户A MPN …” is the complete customer
  // name, not the label “客” followed by a value “户A”. Preserve it verbatim.
  const prefix = line
    .slice(0, mpnIndex)
    .trim()
    .replace(/[|,，;；]+$/, "")
    .trim();
  return new RegExp(`^${token}$`, "u").test(prefix) ? prefix : null;
}
...
function safeRows(
  ...
  if (rows.length > MAX_OUTPUT_ROWS)
    result.issues.push(
      issue("too_many_rows", `候选结果超过 ${MAX_OUTPUT_ROWS} 行，已截断并需要复核`),
    );
  const checked = validateCandidateRows(rows.slice(0, MAX_OUTPUT_ROWS), {
    requireKind: !allowMissingKind,
  });
  result.rows = checked.rows;
  result.issues.push(
    ...checked.issues.map((x) => ({ code: x.code, message: x.message, rowId: x.rowId })),
  );
  for (const row of result.rows) {
    if (row.verification === "visual_only") {
      result.issues.push({
        code: "missing_evidence",
        message: "视觉识别候选必须人工核对",
        rowId: row.id,
      });
    }
  }
  if (result.rows.length === 0) {
    if (!result.issues.length)
      result.issues.push(issue("missing_mpn", "没有识别到可写入的有效型号"));
...
function textRows(text: string, kindHint: ImportKindHint): CandidateRow[] {
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    ...
    const dateMatches = [
      ...rest.matchAll(
        /(?<![\dA-Za-z])((?:20\d{2}|2[3-9]\d{2})\+|(?:20\d{2}|2[3-9]\d{2})(?=\s|$|[,，;；])|\d{2}\+)(?![A-Za-z])/gi,
      ),
    ].filter((match) => {
      const value = match[1] ?? "";
    ...
    if (dateMatches.length) {
      ...
      const packagePrefix =
        rest.slice(0, firstIndex).match(/(\d+(?:\.\d+)?\s*(?:包|packs?)\s*)$/i)?.[1] ?? "";
    ...
    const standardPack =
      rest
        .match(
          /(?:每\s*(?:包|盘|箱)?|per\s*(?:pack|tray|box))\s*(\d+(?:,\d{3})*(?:\.\d+)?)\s*(?:片|pcs?)?/i,
        )?.[1]
        ?.replace(/,/g, "") ?? null;
    const price =
      rest.match(/(?:[$¥￥]\s*\d+(?:\.\d+)?|\d+(?:\.\d+)?\s*(?:元|USD|CNY)|\b\d+\.\d+\b)/i)?.[0] ??
      "";
      ...
      [
        {
          mpn,
          kind,
          qtyRaw: qtyMatch?.[1] ?? null,
          dateCode,
          standardPack,
          priceRaw: price || null,
          priceAmount: priceInfo.amount,
          priceCurrency: priceInfo.currency,
          priceTax: priceInfo.tax,
          isTp: /\bTP\b|目标价|待报价/.test(line),
          leadTimeText,
          warehouse: /香港|\bHK\b/i.test(line) ? "HK" : /坂田|板田/.test(line) ? "坂田" : null,
          channel: line.match(/渠道\s*([\u4e00-\u9fa5A-Za-z0-9_-]{2,30})/)?.[1] ?? null,
          customer: kind === "inquiry" ? inquiryCustomer(line, match.index) : null,
          note: line,
          evidence: { mpn: evidence },
        },
      ],
      { kindHint, sourceText: text },
    )[0];
...
function hasUnattachedTextContext(text: string, rows: CandidateRow[]): boolean {
  ...
  return (
    /\d/.test(remainder) ||
    /供应商|型号|数量|可供|库存|批次|价格|单价|报价|交期|货期|渠道|仓库|现货|supplier|vendor|item\s+code|available|stock|lot|price|delivery|warehouse|maker/i.test(
      remainder,
    )
  );
}

function awaitlessMoney(value: string): {
  amount: number | null;
  ...
  return {
    ...
    currency: /\$|USD/i.test(value) ? "USD" : /¥|￥|CNY|元/i.test(value) ? "CNY" : null,
    tax: null,
    isTp: /TP/i.test(value),
  };
}
...
function mappingFromEnvelope(envelope: Record<string, unknown>): TableMapping[] {
  ...
  return envelope.mappings.flatMap((value) => {
    ...
    const headerRow =
      typeof item.headerRow === "number" && Number.isInteger(item.headerRow)
        ? item.headerRow
        : null;
    const dataStartRow =
      typeof item.dataStartRow === "number" && Number.isInteger(item.dataStartRow)
        ? item.dataStartRow
        : null;
    if (
...
async function tableResult(
  ...
  const checkedMappings = mappings.map((mapping) => {
    ...
    return invalid
      ? {
          ...mapping,
          needsReview: true,
          reason: mapping.reason || "模型返回的列映射超出原表范围或缺少型号列",
        }
      : mapping;
  });
  ...
  const missingSheetMapping = table.sheets.some(
    (sheet) =>
      sheet.rows.length > 1 && !checkedMappings.some((mapping) => mapping.sheet === sheet.name),
  );
  const mappingNeedsReview =
    missingSheetMapping || checkedMappings.some((mapping) => mapping.needsReview);
  if (missingSheetMapping)
    result.issues.push(issue("ambiguous_mapping", "模型没有为每个有数据的工作表返回列映射"));
...
async function modelRows(
  ...
  const raws = Array.isArray(envelope?.rows)
    ? envelope.rows.filter((x): x is Record<string, unknown> => Boolean(x && typeof x === "object"))
    : [];
  return safeRows(
    result,
    normalizeRows(raws, { kindHint: request.kindHint, sourceText: text, visualOnly }),
    request.kindHint === "neutral",
  );
}

export async function extractImport(
  request: ExtractRequest,
  provider: ExtractionProvider = defaultImportProvider(),
): Promise<ExtractionResult> {
  ...
  if (request.source.type === "text") {
    ...
    if (!text.trim()) {
      result.status = "invalid_input";
      result.issues.push(issue("empty_input", "没有可识别的文本"));
      return result;
    }
    if (text.length > MAX_TEXT_CHARS) {
      result.status = "invalid_input";
      result.issues.push(issue("file_too_large", "文本超过20万字符限制"));
      return result;
    }
  ...
  if (request.source.type === "csv" || request.source.type === "excel") {
    try {
      const table =
        request.source.type === "csv"
          ? parseCsv(textOf(request.source.content))
          : await parseExcel(bytes);
      const totalRows = table.sheets.reduce((sum, sheet) => sum + sheet.rows.length, 0);
      if (totalRows > MAX_TABLE_ROWS) {
        result.status = "invalid_input";
        result.issues.push(issue("too_many_rows", `表格超过 ${MAX_TABLE_ROWS} 行限制`));
        return result;
      }
      return tableResult(request, table, provider, result);
    } catch {
      result.status = "invalid_input";
      result.issues.push(issue("invalid_file", "表格文件无法解析"));
      return result;
    }
  }
  if (request.source.type === "docx") {
    if (looksLikeDoc(bytes)) {
      ...
      result.issues.push(issue("unsupported_file", "老式 .doc 不支持，请另存为 .docx 或 PDF"));
      return result;
    }
    try {
      if (!looksLikeDocx(bytes)) {
        result.status = "invalid_input";
        result.issues.push(issue("invalid_file", "文件不是有效 DOCX"));
        return result;
      }
      const text = await extractDocx(bytes);
      if (!text) {
        result.status = "needs_review";
        result.issues.push(issue("empty_input", "DOCX 没有可提取文本"));
        return result;
      }
      return modelRows(request, provider, text, undefined, false, result);
    } catch {
      result.status = "invalid_input";
      result.issues.push(issue("invalid_file", "DOCX 无法解析"));
      return result;
    }
  }
  if (request.source.type === "pdf") {
    try {
      if (!looksLikePdf(bytes)) {
        result.status = "invalid_input";
        result.issues.push(issue("invalid_file", "文件不是有效 PDF"));
        return result;
      }
      const extracted = await extractPdfText(bytes);
      if (extracted && extracted.pageCount > MAX_PDF_PAGES) {
        result.status = "invalid_input";
        result.issues.push(issue("too_many_pages", `PDF超过${MAX_PDF_PAGES}页限制`));
        return result;
      }
      return modelRows(
        request,
      ...
      result.status = "invalid_input";
      result.issues.push(issue("invalid_file", "PDF无法解析"));
      return result;
    }
  }
