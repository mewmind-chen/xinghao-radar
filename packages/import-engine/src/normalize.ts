...
export function parseQty(value: unknown): number | null {
  ...
  const multiplier =
    unit === "万" ? 10000 : unit === "k" ? 1000 : unit === "w" ? 10000 : unit === "m" ? 1000000 : 1;
  return n * multiplier;
}
...
export function parseMoney(value: unknown): {
  ...
  const currency = /\$|USD|美金|美元/i.test(raw)
    ? "USD"
    : /¥|￥|CNY|人民币|元/i.test(raw)
      ? "CNY"
      : null;
  const tax = /含税|含增值税/.test(raw)
    ? "inclusive"
    : /未税|不含税/.test(raw)
      ? "exclusive"
      : null;
...
export function normalizeKind(value: unknown, hint: ImportKindHint): ImportKind | null {
  ...
  if (value === "offer" || value === "inquiry" || value === "stock" || value === "transit")
    return value;
  return null;
}

function clearFieldsOutsideKind(values: CandidateValues, kind: ImportKind | null): void {
  if (kind === "inquiry") {
    values.dateCode = null;
    values.priceTax = null;
    values.leadTimeText = null;
    values.etaText = null;
    values.warehouse = null;
    values.channel = null;
    values.package = null;
    values.standardPack = null;
    values.packState = null;
    values.costAmount = null;
...
function evidenceFor(raw: unknown): Partial<CandidateRow["evidence"]> {
  if (Array.isArray(raw)) {
    ...
    for (const item of raw) {
      ...
      const field =
        typeof value.field === "string" ? (value.field as keyof CandidateValues | "kind") : null;
      const type = value.type;
      if (!field || (type !== "text" && type !== "cell" && type !== "page" && type !== "image"))
        continue;
      ...
      const evidence: SourceEvidence =
        type === "text"
          ...
          : type === "cell"
            ? {
                type,
                sheet: typeof value.sheet === "string" ? value.sheet : "",
                row: typeof value.row === "number" ? value.row : 0,
                column: typeof value.column === "number" ? value.column : 0,
            ...
            : type === "page"
              ? {
                  ...
                  quote,
                  region: Array.isArray(value.region)
                    ? (value.region as [number, number, number, number])
                    : undefined,
                }
              : {
                  type,
...
function hasExactTextEvidence(
  ...
  return (
    hay.includes(normalizedValue) &&
    evidence.some(
      (e) =>
        e.type === "text" &&
        e.quote &&
        hay.includes(e.quote.normalize("NFKC").toLowerCase()) &&
        e.quote.normalize("NFKC").toLowerCase().includes(normalizedValue),
    )
  );
}
...
export function normalizeModelRow(
  ...
  opts: {
    ...
    sourceText?: string;
    visualOnly?: boolean;
    defaultEvidence?: SourceEvidence[];
  },
): CandidateRow | null {
  ...
  values.costAmount =
    typeof raw.costAmount === "number" && Number.isFinite(raw.costAmount)
      ? raw.costAmount
      : cost.amount;
  ...
  clearFieldsOutsideKind(values, kind);
  ...
  const hasTrustedCellEvidence = Boolean(
    evidence.mpn?.some(
      (item) =>
        item.type === "cell" &&
        item.quote?.normalize("NFKC").trim().toLowerCase() === normalizedMpn,
    ),
  );
...
export function normalizeRows(
  ...
  return raws
    .map((raw) => normalizeModelRow(raw, opts))
...
export function exactSourceEvidence(sourceText: string, value: string): SourceEvidence[] {
  const index = sourceText
    .normalize("NFKC")
    .toLowerCase()
    .indexOf(value.normalize("NFKC").toLowerCase());
  return index < 0
    ? []
    : [
        {
          ...
          quote: sourceText.slice(index, index + value.length),
        },
      ];
}
...
export function validateCandidateRows(
  ...
  rows: CandidateRow[];
  issues: {
    code: "missing_mpn" | "missing_kind" | "missing_evidence";
    message: string;
    rowId: string;
  }[];
} {
  ...
  for (const row of rows) {
    ...
    if (requireKind && !row.kind)
      issues.push({ code: "missing_kind", message: "业务类型无法确定", rowId: row.id });
    if (row.verification === "unverified")
      issues.push({ code: "missing_evidence", message: "型号缺少可验证来源", rowId: row.id });
    valid.push(row);
  }
